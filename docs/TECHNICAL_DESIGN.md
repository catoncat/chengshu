...
- 1.16：首页每行可见「更多」；`ACTION_CREATE_DOCUMENT` 保存到用户所选位置，取消不删书。`ImageRepository` 按 URL 缓存图片字节，cancel 只拦后续 load。
- 1.17：同页绝对网址（含未编码中文 fragment）在比较 scheme/host/port/path 后收成 `#id`，不把整段 URL 交给 URI.create。srcset/data-srcset 只在 src 链都空时作为图片来源，取最大 w/x。

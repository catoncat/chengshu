...
装好之后，用当前页面试一次即可。网站上也可以贴链接、贴正文、转本页。当前 Android 版本 **1.16**（覆盖 1.15）。

## 做什么

浏览器分享出来的只是网址，阅读器要的是本地文件。成书挂在系统分享菜单里，接过链接，剥离正文，把文件交给你指定的应用。

格式和打开方式都可以记住，也可以每次再选。重复分享过的链接不会重新抓。切到别的应用后，处理完会通知你（可以拒绝通知，结果仍在首页「需要注意」和「最近」）。首页可以导出或导入 ZIP 备份；导入不会覆盖已经存在、内容不同的书。已保存的每一行可以点「更多」：打开、分享、保存到文件夹、查看原文或删除。

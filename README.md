...
把输入、处理和去向变成可组合的卡牌，让社区贡献能力与完整配方。

从 [三个设计方向与推荐方案](docs/design/mod-bridge/README.md) 开始，查看 [体验与动效分镜](docs/design/mod-bridge/EXPERIENCE.md)，或保存 [可交互 HTML 原型](docs/design/mod-bridge/prototype.html) 后用浏览器打开。

这是独立设计探索：口袋直接用、造物桌试配、工坊改编。原型全部使用固定示例，不执行真实插件、不处理私人内容，也不代表正式 App 已改版。实际检查与未验证项见 [原型验证记录](docs/design/mod-bridge/VALIDATION.md)。
